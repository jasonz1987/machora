...
export function getConfigDir() {
  ...
  const current = path.join(os.homedir(), ".machora");
  const legacy = path.join(os.homedir(), ".rdev");
  return !existsSync(path.join(current, "config.json")) && existsSync(path.join(legacy, "config.json")) ? legacy : current;
}
...
export async function assignProject(metadata, hostTarget) {
  ...
  const project = {
    ...
    devPort: normalizePort(metadata.devPort),
...
export async function updateProjectCommands(projectId, commands) {
  const data = await readStore();
  const project = requireProject(data, projectId);
...
export async function updateProjectHook(projectId, hook) {
  ...
  const project = requireProject(data, projectId);
  project.hook = {
    status: hook?.status === "installed" ? "installed" : "error",
