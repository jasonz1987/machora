  ...
  try {
    ...
    if (platform === "win32") {
      const script = [
        ...
        ...(detailUrl ? [
          `$url = ${powershellQuote(detailUrl)}`,
          "$n.add_BalloonTipClicked({ Start-Process $url })",
        ] : []),
        "$n.Visible = $true",
      ...
      (options.launch || launch)("powershell.exe", ["-NoProfile", "-NonInteractive", "-STA", "-WindowStyle", "Hidden", "-Command", script]);
      return true;
    }
