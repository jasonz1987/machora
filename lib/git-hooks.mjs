...
${marker}
set -u
MACHORA_INPUT=$(mktemp "${shellEscape(path.join(os.tmpdir(), "machora-push.XXXXXX"))}") || exit 0
cat > "$MACHORA_INPUT"
...
MACHORA_CONFIG_DIR=${shellQuote(configDirectory)} nohup ${shellQuote(node)} ${shellQuote(cli)} hook confirm-push --project ${shellQuote(projectId)} --repository ${shellQuote(repository)} --remote "$1" --input "$MACHORA_INPUT" >> ${shellQuote(logPath)} 2>&1 < /dev/null &
exit 0
`;
}
