...
- Keep a quiet global footer at the bottom of the dashboard with the current package version and an accessible link to the Machora GitHub repository.
- Persistent controllers support macOS LaunchAgents and current-user Windows Scheduled Tasks. Windows controller state belongs under `%LOCALAPPDATA%\Machora`, uses the same lifecycle CLI and dashboard flow, and must not require Administrator privileges.
- Keep the Windows Scheduled Task as the default controller mode. Administrator-only Windows features are explicit opt-ins: a Private/Domain inbound firewall rule scoped to the configured Node executable and controller TCP port, plus a WinSW-backed Windows Service with a pinned, SHA-256-verified wrapper. Interactive clickable notifications are guaranteed only in the default logged-in-user mode because Windows Services run in Session 0.
...
- After a project operation or standalone sync is queued successfully, close the project drawer immediately and show a transient confirmation toast that can open the matching Job. Keep the drawer open with an inline error when submission fails.
...
- In project compatibility checks, runtime installation opens a version picker (for example, "Install JDK"), not a fixed distribution. After a tool/runtime installation is submitted from this flow, bind its requirement row to that exact Job, disable duplicate installation, show live installation status/output, and automatically recheck the environment after success without discarding the assignment draft.
...
- Managed Git Hooks must dispatch remote-ref confirmation through a detached Node child on every controller platform; do not depend on `nohup`. Expose status and safe uninstall commands, and restore a preserved user `pre-push` hook during uninstall.
